setwd("/Users/qingtao/Analysis/R/YaleBRCA/TCGAData/InteractionAnalysis/CNVLevel")

library(data.table)
############################################Breast Cancer######################################################
germRecGainCNV<-fread("/Users/qingtao/Analysis/R/YaleBRCA/TCGAData/CNV/EuropeanNormal/PCAEuropNormal_CNV_Recurrent_Gain_cnvGeneMat_geneslevel.txt",sep="\t")
germRecLossCNV<-fread("/Users/qingtao/Analysis/R/YaleBRCA/TCGAData/CNV/EuropeanNormal/PCAEuropNormal_CNV_Recurrent_Loss_cnvGeneMat_geneslevel.txt",sep="\t")
germRareGainCNV<-fread("/Users/qingtao/Analysis/R/YaleBRCA/TCGAData/CNV/EuropeanNormal/PCAEuropNormal_CNV_Rare_Gain_cnvGeneMat_geneslevel.txt",sep="\t")
germRareLossCNV<-fread("/Users/qingtao/Analysis/R/YaleBRCA/TCGAData/CNV/EuropeanNormal/PCAEuropNormal_CNV_Rare_Loss_cnvGeneMat_geneslevel.txt",sep="\t")
...
CEUgermRecGainCNV<-germRecGainCNV[which(germRecGainCNV$sample%in%CEUsample[,1]),]
CEUgermRecLossCNV<-germRecLossCNV[which(germRecLossCNV$sample%in%CEUsample[,1]),]
CEUgermRareGainCNV<-germRareGainCNV[which(germRareGainCNV$sample%in%CEUsample[,1]),]
CEUgermRareLossCNV<-germRareLossCNV[which(germRareLossCNV$sample%in%CEUsample[,1]),]
...
pathInteract<-function(germCNV,somaticSNV,samplename,pathLabel,pathname,pathfile,pvalue = c(0.05, 0.01), fontSize = 1){
  ...
  for(i in 1:length(pathname)){
    pathgene<-pathfile[[i]]
    #germline
    if(length(which(germCNV$Genes%in%pathgene))!=0){
      germlinepathgenemat<-germCNV[which(germCNV$Genes%in%pathgene),]
      GermlineMutSpl<-intersect(germlinepathgenemat$sample,CEUsample[,1])
      if(length(GermlineMutSpl)!=0){
        GermlinePathaltMat[pathname[i],GermlineMutSpl]<-"GermlineCNV"
      }
    }
    #somatic
  ...
  write.csv(GermlinePathaltMat,paste0(pathLabel,"_GermlineCNVMat.csv"))
  write.csv(SomaticPathaltMat,paste0(pathLabel,"SomaticPathaltMat.csv"))
  #oncoprinter: draw pathway mutation map
  require(ComplexHeatmap)
  col = c("GermlineCNV" = "forestgreen", "SomaticMutations" = "slateblue4")
  alter_fun = list(
    background = function(x, y, w, h) {
      grid.rect(x, y, w-unit(0.5, "mm"), h-unit(0.5, "mm"), gp = gpar(fill = "#CCCCCC", col = NA))
    },
    GermlineCNV = function(x, y, w, h) {
  ...
  png(paste0(pathLabel,"_PCA_GermlineCNV_Oncoprinter.png"),height=2000,width=2000,res=96)
  print(oncoPrint(GermlinePathaltMat,  alter_fun = alter_fun,col = col,get_type = function(x) strsplit(x, ";")[[1]], column_title =  paste0("Breast Cancer Germline CNV (",pathLabel,")"),
                  heatmap_legend_param = list(title = "Alternations", at = c("GermlineCNV" , "SomaticMutations"), 
                                              labels = c("GermlineCNV" , "SomaticMutations")
                  )))
  dev.off()
  ...
  GermlinePathaltMat1[which(GermlinePathaltMat1=="GermlineCNV")]=1
  ...
  png(paste0(pathLabel,"_somatic_germlineCNV_interaction.png"),height=1000,width=1000)  
...
pathInteract(germCNV=CEUgermRecGainCNV,somaticSNV=somaticSNVImpact,samplename=CEUsample[,1],pathLabel="HallMark_GermlineRecurrentGain",pathname=names(hallmarkpathway),pathfile=hallmarkpathway)
pathInteract(germCNV=CEUgermRecLossCNV,somaticSNV=somaticSNVImpact,samplename=CEUsample[,1],pathLabel="HallMark_GermlineRecurrentLoss",pathname=names(hallmarkpathway),pathfile=hallmarkpathway)
pathInteract(germCNV=CEUgermRareGainCNV,somaticSNV=somaticSNVImpact,samplename=CEUsample[,1],pathLabel="HallMark_GermlineRareGain",pathname=names(hallmarkpathway),pathfile=hallmarkpathway)
pathInteract(germCNV=CEUgermRareLossCNV,somaticSNV=somaticSNVImpact,samplename=CEUsample[,1],pathLabel="HallMark_GermlineRareLoss",pathname=names(hallmarkpathway),pathfile=hallmarkpathway)



pathInteract(germCNV=CEUgermRareLossCNV,somaticSNV=somaticSNVImpact,samplename=CEUsample[,1],pathLabel="CorePathways_GermlineRareLoss",pathname=names(CorePathway),pathfile=CorePathway)
pathInteract(germCNV=CEUgermRareGainCNV,somaticSNV=somaticSNVImpact,samplename=CEUsample[,1],pathLabel="CorePathways_GermlineRareGain",pathname=names(CorePathway),pathfile=CorePathway)
