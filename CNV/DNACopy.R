...
library(data.table)
rawmat<-as.data.frame(fread(cnvfile,check.names=TRUE))
...
require(DNAcopy)
#CNA.object <- CNA(as.numeric(as.matrix(matrmna[,6])),matrmna$CHROM,matrmna$POS,data.type="logratio",sampleid=spl_name)
CNA.object <- CNA(matrmna[,6],matrmna$CHROM,matrmna$POS,data.type="logratio",sampleid="test_tangent_1")
